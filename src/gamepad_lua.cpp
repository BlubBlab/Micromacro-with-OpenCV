...
int Gamepad_lua::pressed(lua_State *L)
{
	...
	int joyId = (int)lua_tointeger(L, 1) - 1;
	int button = (int)lua_tointeger(L, 2) - 1;
...
int Gamepad_lua::getAxis(lua_State *L)
{
	...
	int joyId = (int)lua_tointeger(L, 1) - 1;
	int axisId = (int)lua_tointeger(L, 2);
...
int Gamepad_lua::getCount(lua_State *L)
{
	if( lua_gettop(L) != 0 )
		wrongArgs(L);
	lua_pushinteger(L, Macro::instance()->getHid()->getGamepadCount());
	return 1;
}
